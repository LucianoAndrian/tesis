...
pp_estaciones = array(NA, dim = c(length(lon2), length(lat2), 28, 12)) 
...
prom_est_cpc_pp = array(NA, dim = c(length(lon2), length(lat2), 28, 4))
i=1
while(i<=4){
  prom_est_cpc_pp[,,,i] = apply(pp_estaciones[,,,(i + 2*i - 2):(i+2*i)], c(1,2,3), mean)
  i = i + 1
}
